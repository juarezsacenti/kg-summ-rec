...
then 
    ...
    docker run --rm -it --gpus all -v "$PWD"/kge-k-means_data:/data -w /data kge-k-means:1.0 /bin/bash -c "python kge-k-means.py --mode multiview --kge $KGE --epochs $EPOCHS --batch_size $BATCH_SIZE --learning_rate $LEARNING_RATE --verbose"
...
then
    ...
    cd util
    python kg2rdf.py --mode 'mv_cluster' --input2 "$HOME/git/datasets/${DATASET}_${KGE}-25/cluster25.tsv" --input "$HOME/git/datasets/${DATASET_IN}/kg.nt" --output "$HOME/git/datasets/${DATASET}_${KGE}-25/kg.nt"
    cd ..
fi
...
then
    ...
    cd util
    python kg2rdf.py --mode 'mv_cluster' --input2 "$HOME/git/datasets/${DATASET}_${KGE}-50/cluster50.tsv" --input "$HOME/git/datasets/${DATASET_IN}/kg.nt"  --output "$HOME/git/datasets/${DATASET}_${KGE}-50/kg.nt"
    cd ..
fi
...
then
    ...
    cd util
    python kg2rdf.py --mode 'mv_cluster' --input2 "$HOME/git/datasets/${DATASET}_${KGE}-75/cluster75.tsv" --input "$HOME/git/datasets/${DATASET_IN}/kg.nt"  --output "$HOME/git/datasets/${DATASET}_${KGE}-75/kg.nt"
    cd ..
fi
