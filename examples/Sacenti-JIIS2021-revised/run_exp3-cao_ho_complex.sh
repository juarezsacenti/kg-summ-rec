...
#######################################
# Run experiment Cao's item-graph hold-out for JIIS2021.
# - Datasets: ml-cao
# - Split: hold-out (ho)
...
#   - Results: summ_impacts, overall_comp_cost
# - KG Recommendation:
#   - Algorithms: CFKG, CKE, CoFM, JtransUp (KTUP); TransE, TransH, BPRMF,
#   TransUp (TUP)
#   - Results:  rec_quality, rec_effectiveness, rec_efficiency
# GLOBALS:
#   HOME
#   experiment: running experiment. Default is 'JIIS2021-revised-exp3'.
# FUNCTIONS:
#   preprocess_cao_oKG
#   preprocess_cao_fKG
#   summarize_cao_sKG
    ...
    do
        ...
        do
            ...
            then
                ...
                if [ "${verbose}" = true ]
                then
                    kge-k-means ${experiment} ${dataset_in} ${dirName} ${kg_filename} ${summarization_mode} ${kge} ${epochs} ${batch_size} ${learning_rate} ${low_frequence} ${ratio} ${relations} ${seed} 'true'
                else
                    kge-k-means ${experiment} ${dataset_in} ${dirName} ${kg_filename} ${summarization_mode} ${kge} ${epochs} ${batch_size} ${learning_rate} ${low_frequence} ${ratio} ${relations} ${seed} 'false'
                fi
                ENDTIME=$(date +%s)
...
recommend_cao_sfKG() {
    ...
    recommend_cao "ml-cao_ho_fKG" "ml-cao_ho_fKG"
...
kg_recommendation() {
    ...
    recommend_cao "${dataset_in}" "${dataset_in}"
