...
recommend_cao() {
    ...
    STARTTIME=$(date +%s)
    if [ "$verbose" = true ]; then echo "[kg-summ-rec] recommend: Running CFKG2 with ${dataset_out}"; fi
    CUDA_VISIBLE_DEVICES=0 nohup python run_knowledgable_recommendation.py -L1_flag -batch_size ${BATCH_SIZE} -data_path ~/git/datasets/${experiment}/${dataset_in}/cao-format/ -dataset ml1m -early_stopping_steps_to_wait 175000 -embedding_size 100 -eval_interval_steps 35000 -nohas_visualization -joint_ratio 0.5 -kg_lambda 1 -kg_test_files valid.dat:test.dat -l2_lambda 0 -learning_rate 0.001 -load_ckpt_file "$HOME/git/results/${experiment}/${dataset_out}/ml1m-bprmf-pretrained1.ckpt:$HOME/git/results/${experiment}/${dataset_out}/ml1m-transe-pretrained2.ckpt" -log_path ~/git/results/${experiment}/${dataset_out}/ -model_type cfkg -negtive_samples 1 -norm_lambda 1 -optimizer_type Adam -rec_test_files valid.dat:test.dat -seed 3 -share_embeddings -topn 10 -training_steps 3500000 -nouse_st_gumbel &
    resource_usage $! 1800 "${HOME}/git/results/${experiment}/${dataset_out}/cfkg2-resource_usage.csv"
    wait $!
    ENDTIME=$(date +%s)
    echo -e "recommend-CFKG2-${dataset_out}\t$(($ENDTIME - $STARTTIME))\t${STARTTIME}\t${ENDTIME}" >> ${overall_comp_cost}

    BATCH_SIZE=600
    #[CFKG] (TransE) - Yongfeng Zhang, Qingyao Ai, Xu Chen, and Pengfei Wang. 2018. Learning over Knowledge-Base Embeddings for Recommendation. In SIGIR.
    STARTTIME=$(date +%s)
    if [ "$verbose" = true ]; then echo "[kg-summ-rec] recommend: Running CFKG3 with ${dataset_out}"; fi
    CUDA_VISIBLE_DEVICES=0 nohup python run_knowledgable_recommendation.py -L1_flag -batch_size ${BATCH_SIZE} -data_path ~/git/datasets/${experiment}/${dataset_in}/cao-format/ -dataset ml1m -early_stopping_steps_to_wait 175000 -embedding_size 100 -eval_interval_steps 35000 -nohas_visualization -joint_ratio 0.5 -kg_lambda 1 -kg_test_files valid.dat:test.dat -l2_lambda 0 -learning_rate 0.001 -load_ckpt_file "$HOME/git/results/${experiment}/${dataset_out}/ml1m-bprmf-pretrained1.ckpt:$HOME/git/results/${experiment}/${dataset_out}/ml1m-transe-pretrained2.ckpt" -log_path ~/git/results/${experiment}/${dataset_out}/ -model_type cfkg -negtive_samples 1 -norm_lambda 1 -optimizer_type Adam -rec_test_files valid.dat:test.dat -seed 3 -share_embeddings -topn 10 -training_steps 3500000 -nouse_st_gumbel &
    resource_usage $! 1800 "${HOME}/git/results/${experiment}/${dataset_out}/cfkg3-resource_usage.csv"
    wait $!
    ENDTIME=$(date +%s)
    echo -e "recommend-CFKG3-${dataset_out}\t$(($ENDTIME - $STARTTIME))\t${STARTTIME}\t${ENDTIME}" >> ${overall_comp_cost}
