...
def kge_clustering(data_home, folder, triples_file, items_file, mode, kge_name, rates, verbose):
    # Load triples:
    ...
    # Select mode:
    if mode == 'singleview':
        singleview(triples, items, kge_name, rates, verbose)
    elif mode == 'multiview':
        multiview(triples, items, kge_name, rates, verbose)
    else:
        sys.exit('Given mode is not valid.')
    

def singleview(triples, items, kge_name, rates, verbose):
    ...
    # Train KGE model
    model = kge(triples, kge_name, verbose)
...
def multiview(triples, items, kge_name, rates, verbose):
    # Select all entities, except items
    triples_df = pd.DataFrame(triples, columns=['s', 'p', 'o'])
...
if __name__ == '__main__':
    ...
    kge_clustering(data_home, folder, triples_file, items_file, mode, kge_name, rates, verbose)
