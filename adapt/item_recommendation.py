...
def case_rec_evaluate(FLAGS, model, eval_iter, eval_dict, all_dicts, logger, i, eval_descending=True, is_report=False):
    ...
    dataset_name = os.path.basename(os.path.dirname(os.path.dirname(FLAGS.log_path)))
    ...
    for _ in range(trainer.step, FLAGS.training_steps):
        ...
        if trainer.step % FLAGS.eval_interval_steps == 0 :
            ...
            for i, eval_data in enumerate(eval_datasets):
                ...
                #case_rec_evaluate(FLAGS, model, eval_data[0], eval_data[3], all_eval_dicts, logger, i, eval_descending=True if trainer.model_target == 1 else False, is_report=is_report)
